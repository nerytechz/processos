...
2. **Inicializar `cursor.md` como a Constituição do Projeto:** 
...
Você está estritamente proibido de escrever scripts em `tools/` até que:
    ...
    - O Esquema de Dados seja definido em `cursor.md`.
    - O `task_plan.md` tenha um Blueprint aprovado.

---

### 🧪 **Protocolo Spike — Pausa Controlada para Prova de Conceito**

Em casos excepcionais, quando uma **incerteza técnica crítica** bloqueia a Visão (ex.: "será que o knob consegue simular scrub do áudio?"), o protocolo pode ser **pausado temporariamente** para um spike isolado.

**Regras obrigatórias:**

1. **Escopo mínimo:** Apenas o comportamento em dúvida. Sem UI de produto, sem documentação de processos, sem integrações definitivas.
2. **Local efêmero:** Todo código do spike vive **somente** em `.tmp/spike-*/`. Proibido criar regras em `cursor.md`, POPs em `architecture/` ou scripts em `tools/`.
3. **Autodestruição:** Ao encerrar o spike, **apagar** a pasta `.tmp/spike-*/` por completo. Nenhuma regra, schema ou decisão do spike entra na constituição do projeto.
4. **Registro permitido:** Apenas `progress.md` e `findings.md` recebem o **resultado do teste** (funcionou / não funcionou / limitações). Isso não vira lei — é memória de pesquisa.
5. **Retomada:** Quando o usuário pedir para retomar o V.L.A.E.G., continuar **exatamente de onde parou** (fase, pergunta pendente, checklist). O spike não substitui Protocolo 0 nem pula etapas seguintes.

**Gatilho de pausa:** Usuário aprova explicitamente a pausa + objetivo do spike.  
**Gatilho de retomada:** Usuário pede retomar o protocolo após o teste + pasta do spike deletada.
...
2. **Regra de Dados Primeiro:** Você deve definir o JSON Data Schema (formatos de Entrada/Saída) em `cursor.md`. A codificação só começa quando o formato do "Payload" for confirmado.
...
3. **Documentação:** Finalize o Log de Manutenção em `cursor.md` para estabilidade a longo prazo.
...
Antes de construir qualquer Ferramenta, você deve definir o Esquema de Dados em `cursor.md`.
...
- Apenas atualize `cursor.md` quando: Um esquema mudar, uma regra for adicionada ou a arquitetura for modificada.
- `cursor.md` é a lei. Os arquivos de planejamento são a memória.
...
`├── cursor.md          # Mapa do Projeto e Rastreamento de Estado
